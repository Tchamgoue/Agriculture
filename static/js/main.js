...
//            $('#myModalExperience').find("select.notice_period").filter(function() {
//                            return $(this).text() == $.trim($(this).closest("tr").find('td:eq(6)').text());
//            }).prop('selected', true);
//            $('#myModalExperience').find(".notice_period").text($.trim($(this).closest("tr").find('td:eq(6)').text()))
        ...
        // Click event For Adding Certifications By Applicants
        // Validation on the models can/should be improved later on
...
$(function(){
    $("select[name='job_occupation_group_list_unique']").change(
    function(){
        var $select = $("select[name='job_fields_work_list_unique']");
        $select.find("*").hide();
        if($(this).val() != null) {
            for(var i = 0; i < $(this).val().length; i++) {
                var nb = $select.find("option[job-occupation_id="+($(this).val()[i] || 0)+"]").show().size()
            }
            $select.val(0)
        }
    }
...
$(function(){
    $("select[name='job_occupation_unique']").change(
    function(){
        var $select = $("select[name='job_fields_work_unique']");
        $select.find("*").hide();
        if($(this).val() != null) {
            var nb = $select.find("option[job-occupation_id="+($(this).val() || 0)+"]").show().size();
            $select.val(0)
        }
    }
  )
