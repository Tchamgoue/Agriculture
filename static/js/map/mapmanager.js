...
if (demo == true) {
    var colormap = L.map("map", {
        center: [10.0268688, 5.4527263],
        zoom: 34
    }).flyTo([10.0268688, 5.4527263], 7);
}
else {
    var colormap = L.map("map", {
...
(function ($) {
    $(function (e) {
        ...
        if (demo == true) {
            var marker = L.marker([10.0268688, 5.4527263])
        }
        else {
    ...
    $("#form-similarity").on("submit", function (evt) {
        ...
        legend.onAdd = function (map) {
            ...
            let length=demo_color.length;
            let to_add=parseInt(100/length);
            for (var i = 0; i < length; i++) {

                div.innerHTML += "<i style='background:" + demo_color[i] + "'></i>" +(i*to_add)+"-"+(i+1)*to_add  + "%<br />";
            }
            return div;
        }
        ...
        $.ajax(settings).done(function (response) {
            ...
            console.log("Success");
            [0, 1, 2, 3, 4]
            legend.addTo(colormap,demo_color);
            if (demo == true) {
                ...
                for (var i = 0; i < response["point"].length; i++) {
                    ...
                    var circle = L.circle([response["point"][i]["long"], response["point"][i]["lat"]], {
                        radius: 50, color: demo_color[trouve],
                ...
                //alert(random_point)
                colormap.flyTo([response["point"][random_point]["long"], response["point"][random_point]["lat"]], 10);

            }
            else {
                for (var i = 0; i < response.length; i++) {
                    var circle = L.circle([response[i][0], response[i][1]], {
                        radius: 10000, color: 'green',
                        fillColor: 'green',
                        fillOpacity: 0.5
                    }).addTo(colormap);
                }
